    ...
    >
      ...
      <el-table-column label="属性内容">
        <template slot-scope="{row}">
          <el-tag v-for="(propvalue,index) in row.propvalues" :key="index" style="margin-right:10px;">{{ propvalue.name }}</el-tag>
        </template>
      </el-table-column>
...
import FormDialog from './components/prop-dialog'
...
export default {
  ...
  methods: {
    async loadList() {
      this.loading = true
      const res = await this.$api.prop.list({
        ...this.query,
        ...this.pagination
      })
    ...
    async deleteData(row) {
      const res = await this.$api.prop.delete({ id: row.id })
      this.loadList()
