...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="isAdmin" prop="isAdmin">
      ...
      <el-table-column label="permissions">
        <template slot-scope="{row}">
          <div v-for="(item,index) in row.permissions" :key="index">{{ item }}</div>
        </template>
      </el-table-column>
...
import FormDialog from './components/role-dialog'
...
export default {
  ...
  methods: {
    async loadList() {
      this.loading = true
      const res = await this.$api.shop_role.list({
        ...this.query
        // ...this.pagination
      })
    ...
    async deleteData(row) {
      const res = await this.$api.shop_role.delete({ id: row.id })
      this.loadList()
